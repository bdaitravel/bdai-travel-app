indicar que esto no esta productivo y es solo una posible mejora no implantada
...
// ── Buscar entrada principal de edificio grande (tag entrance=main/yes en OSM) ─
// Solo se activa si el tipo de parada corresponde a un edificio visitable.
// Protocolo GIS Core: priorizar la fachada/entrada principal sobre el centroide del polígono,
// que en edificios grandes (catedrales, palacios, museos) puede caer en patio interior o cubierta.
const BUILDING_STOP_TYPES = new Set(['architecture', 'historical', 'art', 'culture']);

const fetchMainEntrance = async (lat: number, lon: number, stopName: string): Promise<{ lat: number; lon: number } | null> => {
    // Bbox de ≈120m alrededor del centroide verificado
    const delta = 0.0011;
    const bbox = `${lat - delta},${lon - delta},${lat + delta},${lon + delta}`;
    const query = `[out:json][timeout:8];node["entrance"~"main|yes"](${bbox});out body;`;
    ...
    try {
        ...
        // Preferir `entrance=main` sobre `entrance=yes` si coexisten
        const mainNodes = nodes.filter(n => n.tags?.entrance === 'main');
        const candidates = mainNodes.length > 0 ? mainNodes : nodes;

        // Si hay varios candidatos, elegir el más cercano al centroide
        ...
        console.log(`[GIS] 🚪 Entrada principal: "${stopName}" → (${best.lat.toFixed(6)}, ${best.lon.toFixed(6)}) ajuste=${(bestDist * 1000).toFixed(0)}m desde centroide`);
        return { lat: best.lat, lon: best.lon };

    } catch (e) {
        console.warn(`[GIS] fetchMainEntrance failed para "${stopName}":`, e);
        return null;
    }
};
...
// municipios adyacentes pasen el filtro de radio.
// FACHADA: tras verificar coordenadas, si el tipo es edificio, se consulta entrance=main en OSM.
        ...
        if (nomRes.ok) {
            ...
            for (const result of (data || [])) {
                ...
                if (distToCenter <= radiusKm) {
                    ...
                    // ── FACHADA PRINCIPAL: solo para tipos edificio ──────────────
                    if (BUILDING_STOP_TYPES.has(stop.type)) {
                        const entrance = await fetchMainEntrance(lat, lon, stop.name);
                        if (entrance) {
                            return { ...stop, latitude: entrance.lat, longitude: entrance.lon, coordinatesVerified: true, entranceResolved: true };
                        }
                    }
...
serve(async (req) => {
    try {
        ...
        // Log resumen con conteo de fachadas resueltas
        const totalEntrancesResolved = processedTours
            .flatMap(t => t.stops)
            .filter(s => s.entranceResolved).length;
        console.log(`[GIS] ✅ Completado. Tours: ${processedTours.length}. Paradas totales: ${totalUniqueStops.length}. Fachadas resueltas: ${totalEntrancesResolved}.`);
