// services/supabase/tour-worker-gis-02.md
// ESTE ARCHIVO ES LA FUENTE DE LA VERDAD (SSOT) PARA LA EDGE FUNCTION 'tour-worker-gis-02'
// Recibe el webhook de UPDATE en generation_jobs (status=PENDING_GIS_02),
...
const verifyStopCoordinates = async (stop: any, city: string, country: string, cityInfo: any): Promise<any | null> => {
    ...
    const radiusKm = Math.max(20, cityInfo.radiusKm || 20);
    const normalizedCity = city.toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g, '').trim();

    // Extraer nombre principal y alias entre paréntesis para geocodificar por separado.
    // "Alameda de Cervantes (Parque de la Dehesa)" → mainName="Alameda de Cervantes", alias="Parque de la Dehesa"
    const parenMatch = stop.name.match(/^(.+?)\s*\(([^)]+)\)\s*$/);
    const mainName = parenMatch ? parenMatch[1].trim() : stop.name;
    const aliasName = parenMatch ? parenMatch[2].trim() : null;
    // Intentar mainName primero; si hay alias, intentarlo como segunda opción independiente
    ...
    try {
        ...
        // VALIDACIÓN DE MUNICIPIO: descartar resultados de municipios vecinos con el mismo
        // nombre de calle/lugar (ej. Calle Laurel en Villamediana vs Logroño).
        await sleep(1100); // Rate limiter Nominatim (max 1 req/s)
        const nominatimQuery = `${mainName}, ${city}${country ? ', ' + country : ''}`;
        const nomUrl = `https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(nominatimQuery)}&format=json&limit=5&addressdetails=1`;
        ...
        // ── Intento 3: Photon con validación de municipio (FALLBACK 2) ────────────
        await sleep(500);
        const photonUrl = `https://photon.komoot.io/api/?q=${encodeURIComponent(nominatimQuery)}&limit=5&lang=en`;
...
serve(async (req) => {
    try {
        ...
        if (job.status !== 'PENDING_GIS_02') {
            return new Response('Job is not PENDING_GIS_02', { status: 200 });
        }
        ...
        // { city, country, lat, lon, radiusKm, population, bbox:{south,west,north,east} }
        const cityInfo = job.city_info || {};
        const city = cityInfo.city || job.city_slug.split('_')[0]; // fallback
        const country = cityInfo.country || 'Spain'; // fallback
        ...
        for (let i = 0; i < rawTours.length; i++) {
            ...
            for (const stop of tour.stops) {
                const verifiedStop = await verifyStopCoordinates(stop, city, country, cityInfo);
                if (verifiedStop) {
        ...
        // ── DAI Empty Tour Text ──────────────────────────────────────────────────────
        const buildEmptyTour = (citySlug: string, language: string, cityName: string): any => {
            const isEnglish = language.startsWith('en');
            const isFrench = language.startsWith('fr');
            const isGerman = language.startsWith('de');
            const isItalian = language.startsWith('it');

            let title = "Un rincón demasiado tranquilo";
            let desc = `Sinceramente, he buscado por todos los rincones de ${cityName} y no he encontrado suficientes lugares destacables como para justificar un paseo en condiciones. Podría inventarme un tour, pero mi programación (y mi orgullo) me lo impiden. Te recomiendo sentarte a disfrutar de la paz, porque turismo activo... aquí no hay.`;

            if (isEnglish) {
                title = "A corner a bit too quiet";
                desc = `Honestly, I've scoured every corner of ${cityName} and couldn't find enough notable spots to justify a proper walk. I could make things up, but my programming (and my pride) won't allow it. I recommend you sit back and enjoy the peace, because active sightseeing... just isn't happening here.`;
            } else if (isFrench) {
                title = "Un coin un peu trop calme";
                desc = `Honnêtement, j'ai fouillé tous les recoins de ${cityName} et je n'ai pas trouvé assez de lieux remarquables pour justifier une vraie promenade. Je pourrais inventer des choses, mais ma programmation (et ma fierté) me l'interdisent. Je vous recommande de vous asseoir et de profiter du calme.`;
            } else if (isGerman) {
                title = "Ein etwas zu ruhiges Fleckchen";
                desc = `Ehrlich gesagt habe ich jeden Winkel von ${cityName} abgesucht und nicht genug bemerkenswerte Orte gefunden, um einen richtigen Spaziergang zu rechtfertigen. Ich könnte mir Dinge ausdenken, aber meine Programmierung (und mein Stolz) verbieten mir das. Genießen Sie einfach die Ruhe.`;
            } else if (isItalian) {
                title = "Un angolo un po' troppo tranquillo";
                desc = `Sinceramente, ho cercato in ogni angolo di ${cityName} e non ho trovato abbastanza luoghi di rilievo per giustificare una vera passeggiata. Potrei inventarmi delle cose, ma la mia programmazione (e il mio orgoglio) me lo impediscono. Ti consiglio di sederti e goderti la pace.`;
            }

            return {
                id: `${citySlug}_${language}_empty`,
                title,
                description: desc,
                theme: 'empty',
                city: cityName,
                country: citySlug.split('_').slice(1).join('_') || '',
                duration: "0 min",
                distance: "0 km",
                stops: [],
                isEmptyTour: true
        ...
        if (totalUniqueStops.length < 4) {
            console.log(`[GIS] ⚠️ Not enough valid stops (${totalUniqueStops.length} < 4). Generando tour vacío de DAI para ${city}.`);
            
            const emptyTour = buildEmptyTour(job.city_slug, job.language, city);
            ...
            await supabaseClient.from('generation_jobs').update({ status: 'COMPLETED', error_message: 'Not enough valid stops, returned DAI empty tour.' }).eq('id', job.id);
            return new Response('Completed (Empty Tour)', { status: 200 });
        }
