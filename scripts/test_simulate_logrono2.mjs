...
(async () => {
    ...
    beforeNames.forEach((n,i) => console.log(` ${i+1}. ${n}`));

    const optimized = optimizeStops(tour.stops);
    const afterNames = optimized.map(s => s.name);
    console.log('\nORDEN CORREGIDO (Algoritmo arreglado sin salto atrás y sin cluster-star-pattern):');
    afterNames.forEach((n,i) => {
        const oldIdx = beforeNames.indexOf(n) + 1;
        console.log(` ${i+1}. ${n} \t(antes era la parada nº ${oldIdx})`);
    });
