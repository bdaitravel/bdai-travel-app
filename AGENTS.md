...
- **Estructura Temática de 3 Niveles (Obligatoria):**
  ...
  - **Tour 2 — "Alma Local":** Patrimonio vivo, monumentos secundarios valiosos, arquitectura civil, miradores urbanos, patrimonio industrial/regional (ej. trujales antiguos, calados, bodegas centenarias). Sin negocios comerciales genéricos. EXCEPCIÓN: negocios que son patrimonio cultural en sí mismos (bodegas Franco-Españolas, Guinness Factory, tiendas centenarias de fama nacional/mundial).
  - **Tour 3 — "Secretos y Curiosidades":** Placas olvidadas, leyendas urbanas verificadas, detalles arquitectónicos curiosos, anécdotas que ni los lugareños conocen.
- **Rango de Paradas:** Objetivo de 12 paradas por tour (mínimo viable absoluto: 4). Número de tours determinado por **paradas verificadas reales** (no por población): <16 stops → 1 tour; 16-23 → 2 tours; 24+ → 3 tours.
- **Lógica de Rescate (CRÍTICO):** Si tras la verificación GIS individual los tours quedan con pocas paradas, se activará la "Lógica de Rescate": si el total de paradas únicas verificadas es >= 4 pero < 16, se fusionarán todas en un único tour robusto. Si el total es < 4, la generación se considerará fallida.
...
- **Ejecución Asíncrona (Edge Functions) y Plan Pro:** Para sortear el límite estricto de **150 segundos de "request idle timeout"** en las integraciones HTTP (Límite oficial para el **Plan Pro de Supabase**), la generación pesada de Gemini y validación GIS corre en una tarea en background (`runBackgroundGeneration`). La petición HTTP retorna inmediatamente el estado `BACKGROUND_STARTED`, dándole a la tarea pesada un margen de hasta **400 segundos de "wall clock limit"** (el límite máximo global de ejecución en el Plan Pro) para terminar su proceso sin bloquear al cliente.
