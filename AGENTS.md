...
- **Tour Seeder (`scripts/seedTours.ts`):** Script Node.js (TypeScript) para pre-poblar la caché de tours + audio sin intervención de usuario. Ejecutar con `npm run seed` (usa `npx tsx` internamente).
  - **Entrada:** `scripts/cities.txt` — una línea por ciudad en formato `"Ciudad, País"` (nombres en inglés). Las líneas `#` son comentarios.
  - **Flujo por ciudad:** (1) Verificar si ES ya está `READY` en `tours_cache`; si no, llamar `tour-orchestrator` y sondear cada 15s hasta `READY` (máximo 15 min). (2) Llamar `generate-audio-dai` para cada `stop.description` en ES (dedup por hash en la EF). (3) Si EN no existe, traducir el JSON ES completo con Gemini (`gemini-2.5-flash`, `responseMimeType: 'application/json'`): solo los campos text (title, description, theme, stop.name, stop.description), reemplazando `_es_` → `_en_` en IDs; el resto del objeto se preserva. (4) Upsert directo a `tours_cache` con `service_role`. (5) Generar audio EN igual que ES.
  - **Variables de entorno requeridas:** `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY` (para Edge Functions), `SUPABASE_SERVICE_ROLE_KEY` (para writes a `tours_cache`), `VITE_GEMINI_API_KEY`.
  - **Idempotente:** Cada paso comprueba si ya está hecho antes de actuar. Re-ejecutar es seguro.
  - **Rate limiting:** 400ms entre llamadas de audio, 2s de cooldown entre ciudades.
