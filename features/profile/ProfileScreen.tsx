...
export const ProfileScreen: React.FC<ProfileScreenProps> = ({ user, onClose, onUpdateUser, onLogout, onOpenAdmin }) => {
  ...
  const handleSave = async () => {
    setIsSyncing(true);
    try {
      const birthDate = new Date(formData.birthday || '1995-01-01');
      const age = new Date().getFullYear() - birthDate.getFullYear();
      const updatedUser = { ...user, ...formData, age };
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="bg-[#f3f0e6] w-full max-w-sm rounded-[2.5rem] overflow-hidden shadow-2xl relative z-10 border-[3px] border-[#d7d2c3] flex flex-col max-h-[95vh] text-slate-900">
        ...
        <div className="bg-[#8b2b2b] p-5 flex justify-between items-center shrink-0">
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar p-6 space-y-6">
          <div className="flex gap-6 items-start">
            <div className="shrink-0 space-y-2">
              <div onClick={handleAvatarChange} className="w-28 h-36 bg-white border-2 border-[#d7d2c3] rounded-xl shadow-lg overflow-hidden p-1 relative cursor-pointer group">
                <img src={formData.avatar} className="w-full h-full object-cover grayscale contrast-125 saturate-0" />
                <div className="absolute inset-0 bg-black/40 flex flex-col items-center justify-center text-white text-[8px] font-black text-center opacity-0 group-hover:opacity-100 transition-opacity gap-1">
                  <i className="fas fa-sync-alt"></i>
                  {t('profile.changeAvatar')}
                </div>
              </div>
              <button onClick={() => fileInputRef.current?.click()} className="w-full py-2 bg-slate-900 text-white rounded-lg font-black uppercase text-[8px] tracking-widest flex items-center justify-center gap-2">
          ...
          <div className="pt-2">
            ...
            <div className="flex flex-wrap gap-2 mb-8">
              {LANGUAGES.map(lang => (
                <button 
                  ...
                  className={`w-8 h-8 rounded-full border flex items-center justify-center transition-all ${language === lang.code ? 'bg-purple-600 text-white scale-110 shadow-lg' : 'bg-white text-slate-400'}`}
                >
            ...
            <button onClick={onLogout} className="w-full py-4 border border-red-100 text-red-500 text-[9px] font-black uppercase tracking-[0.2em] flex items-center justify-center gap-2 rounded-2xl">
