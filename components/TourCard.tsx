...
export const ActiveTourCard: React.FC<any> = ({ tour, user, currentStopIndex, onNext, onPrev, onJumpTo, onUpdateUser, onBack, language = 'es', userLocation }) => {
    ...
    const handlePlayAudio = async (stopId: string, text: string) => {
        ...
        try {
            ...
            if (e.name === "QuotaError" || e.message?.includes("Límite excedido")) {
                const hasKey = await window.aistudio.hasSelectedApiKey();
                if (!hasKey) {
                    if (confirm("Has alcanzado el límite de la versión gratuita. ¿Quieres añadir tu propia clave API de Google Cloud (gratis) para continuar?")) {
                        await window.aistudio.openSelectKey();
                    }
                } else {
                    alert(e.message || "Límite de cuota alcanzado.");
                }
            } else {
